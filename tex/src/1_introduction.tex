...
\begin{DefGrey}\label{CFGinCNF} \textbf{CFG with restrictions}\\
	...
	\end{itemize}
\end{DefGrey}
\noindent Throughout this thesis a grammar is always synonymous with Definition \ref{CFGinCNF}. $P$ is not necessarily in CNF because it is possible that there are unreachable variables \textendash~from the starting variable. For further convenience the following default values are always true:
\begin{itemize}
	\item $V = \{A, B, ...\}$
...
\begin{DefGrey}\label{wordLanguage} \textbf{Word $w$ and language $L(G)$}
	\begin{itemize}
		\item Word: $w \in \Sigma^* = \{w_0,~w_1,~...,~w_j\}$.
...
Two basic approaches that may help finding a good algorithm are explained informally \cite{Duda.2012}.
...
\end{DefGrey}
\noindent It is called Forward Problem, if you are given a grammar $G$ and form a derivation from its root node to a final word $w$. The derived word $w$ is always element of $L(G)$.
\begin{DefGrey}
...
\end{figure}
\noindent Lastly we define $CellDown$, $CellUpperLeft$ and $CellUpperright$ that are used for description in Algorithm \ref{checkForceCombinationPerCell}.
...
The idea is to find all possible derivations of each subword starting with size one and to consecutively use this information to find all possible derivations with a lager size of the subword up to the size of $w$. Finally it is checked if $w \in L(G)$ through the presence of the start variable in the tip of the pyramid.. \\
The description of the algorithm follows \cite{Hoffmann.2015} adjusted to the data structure $Pyramid$. While describing Algorithm \ref{checkForceCombinationPerCell} a back reference to the CYK-Algorithm will be made that points out a similarity.\\
...
During the execution of Algorithm \ref{CYK} the parsing table is filled as shown in Figure \ref{exampleCYK}.
\noindent
\begin{figure} [h]
	\begin{minipage}{6in}
		\centering
...
\noindent Different Success Rates are used to compare the algorithms accounting to their performance to the different requirements. Here $N \in  \mathbb{N}^+$ is the sample size of all generated grammars of the examined algorithm. Before defining a overall Success Rate ($SR$) three other Success Rates set the basis for it.\\
...
It holds: SR-Producibility $ = p / N$, whereas $p$ is the count of $exercise$s that fulfil the requirement.\\
...
It is true: SR-Cardinality-Rules $ = cr / N$, whereas $cr$ is the count of $exercise$s.\\
...
\end{enumerate}
It holds: SR-Pyramid $= p / N$, whereas $p$ is the count of $exercise$s that fulfil the three requirements above. While checking 1., 2. and 3. a simplification of $Cell_{i,j}$ is done:\\ $Cell_{i,j} \subseteq \{(V,k)~|~k \in \mathbb{N} \} \longrightarrow Cell_{i,j} \subseteq V$, see Figure \ref{simplification}. \\
\noindent 
\begin{figure}[h]
...
\noindent Students easily find a pattern of how to fill the first two rows of the $pyramid$ during execution of the CYK algorithm but do more mistakes starting at row $i\geq2$. The approach of finding only patterns and not thoroughly understanding the algorithm is countered through Algorithm \ref{checkForceCombinationPerCell}. Students often don't know what cell combinations need to taken into account while filling one specific cell of the pyramid. They simply take the $UpperLeftCell$ and the $UpperRightCell$ and try to find rules in the grammar that match the resulting compound variables. More detail to how force a correct cell combination (1.) see Algorithm \ref{checkForceCombinationPerCell}. But note that a right cell combination can only be forced of cells with index $i>1$.\\
...
\noindent As seen in Figure \ref{foreCombination} the variables in $Cell_{2,0}$ and in $Cell_{2,1}$ force each a right cell combination and in both cases $VarComp=\{SS\}$. The variable $v=C$ doesn't have $SS$ as one of its rhses. Therefore the variable $C$ forces.
$Cell_{3,0}$ doesn't force because $VarComp=\{CC\}$ and the variable $v=S$ has $CC$ as its rhse. Remember again that cells with index $i\leq1$ can't force at all.
\noindent
\begin{figure} [h]
	\begin{minipage}{6in}
		\centering
...
\end{figure}\\
\noindent Now that the three Success Rates are known the overall Success Rate can be specified. \\
