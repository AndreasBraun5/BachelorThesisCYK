...
\noindent \frame{
	\begin{algorithm}[H] %or another one check
		...
		\label{CYK}
		\SetAlgoLined
		\KwIn{Grammar $G=(V,\ \Sigma,\ S,\ P)$ and word $w \in \Sigma^* = \{w_0,~w_1,~...,~w_i\}$ }
		\KwOut{true $\Leftrightarrow w\in L(G)$}
		$Pyramid = \emptyset $\;
		\For{$i:=0 \to i_{max}$}{
			$Pyramid \cup \{(X,i)~|~X \longrightarrow w_i\}$
		}
		\For{$i:=1 \to i_{max}$}{
			\For{$j:=0 \to j_{max,i}$}{
				\For{$k:=i-1 \to 0$}{
					$Pyramid \cup \{X\ |\ X\longrightarrow YZ,\ Y \in Cell_{k,j},\ Z \in Cell_{i-k-1,k+j+1} \}$\;
				}
			}
		}
		...
		$V_{i,j} = \emptyset $;~~\tcp{$P \subseteq V \times (V^{2} \cup \Sigma)$}
		\Return $V_{i,j}$\;
	\end{algorithm}
}
