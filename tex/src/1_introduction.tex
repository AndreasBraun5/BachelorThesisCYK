...
\end{DefGrey}
\noindent If you are given a word $w$ and want to determine if it is element of $L(G)$, it is called Backward Problem or parsing.The CYK does parsing and is the focus here.
...
The Cocke-Younger-Kasami Algorithm (CYK) has been developed independently in the 1960s by Itiroo Sakai \cite{Sakai.1962}, John Cocke and Jacob Schwartz \cite{JohnCockeJacobT.Schwartz.1970}, Tadao Kasami \cite{Kasami.1966} and Daniel Younger \cite{YOUNGER.1967} that uses the principle of dynamic programming\cite{wikiCYK}.\\
...
\noindent \frame{
	\begin{algorithm}[H] %or another one check
		...
		\For{$j:=0 \to i_{max}$}{ \label{row1}
			$Pyramid~\cup  = \{(X,j+1)~|~X \longrightarrow w_j\}$; \tcp{Fills cells $Cell_{0,j}$}
		}
		\For{$i:=1 \to i_{max}$}{ \label{rows}
			\For{$j:=0 \to j_{max,i}$}{ \label{eachCell}
				\For{$k:=i-1 \to 0$}{ \label{cykCellCombs}
					$Pyramid~\cup \{(X,k)\ |\ X\longrightarrow YZ,\ Y \in Cell_{k,j},\ Z \in Cell_{i-k-1,k+j+1} \}$; \tcp{Fills cells $Cell_{i,j}$ ??k?? XXX}
				}
			}
		}
...
\noindent The Success Rates ($SR$) are used to compare the algorithms accounting to their performance to the different requirements. $N \in  \mathbb{N}^+$ is the sample size of all generated grammars of the examined algorithm.\\
...
It is true: SR-Cardinality-Rules $ = cr / N$, whereas $cr$ is the count of $exercises$.\\
...
\end{enumerate}
It holds: SR-Pyramid $= p / N$, whereas $p$ is the count of $exercises$ that fulfil the three requirements above.\\
