		...
		\For{$i:=1 \to i_{max}$}{ \label{rows}
			\For{$j:=0 \to j_{max,i}$}{ \label{eachCell}
				\For{$k:=i-1 \to 0$}{ \label{cykCellCombs}
					$Pyramid~\cup \{(X,k)\ |\ X\longrightarrow YZ,\ Y \in Cell_{k,j},\ Z \in Cell_{i-k-1,k+j+1} \}$; \tcp{Fills cells $Cell_{i,j}$ \label{fill2} ??k?? XXX and $Y \in Cell_{k,j}$ is wrong like in circled D}
				}
			}
		}
