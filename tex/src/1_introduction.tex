...
\end{DefGrey}
\noindent "Bottom-Up parsing is the general method used in the Cocke-Younger-Kasami(CYK) algorithm, which fills a parse table from the "bottom up""\cite{Duda.2012}.
\begin{DefGrey}
...
Reasonably criteria to guide the choice of which rewrite rule to apply could include to begin the parsing at the first (left) or last (right) character of the word $w$ \cite{Duda.2012}.
...
The Cocke-Younger-Kasami Algorithm (CYK) has been developed independently in the 1960s by Itiroo Sakai \cite{Sakai.1962}, John Cocke and Jacob Schwartz \cite{JohnCockeJacobT.Schwartz.1970}, Tadao Kasami \cite{Kasami.1966} and Daniel Younger \cite{YOUNGER.1967} that uses the principle of dynamic programming.\\
The description of the algorithm follows \cite{Hoffmann.2015} adjusted to the help data structure $Pyramid$. \\
...
An generated $exercise$ contributes to the SR-Producibility iff the CYK algorithm's output (Algorithm \ref{CYK}) is true, in other words $w\in L(G)$.\\
...
\noindent For more detail to how a cell forces a right cell combination (1.) see the following algorithm. But note that a right cell combination can only be forced of cells with index $i>1$. This restriction while calling Algorithm \ref{checkForceCombinationPerCell} is only thoughtful in the context of a exercise. Students easily find a pattern of how to fill the first two rows of the $pyramid$ but do more mistakes beginning at row $i\geq2$. The approach of finding only patterns and not thoroughly understanding the algorithm is countered that way.\\
...
\begin{figure} [h]
	...
	\end{minipage}
	\caption{Application of Algorithm \ref{checkForceCombinationPerCell} onto a entire pyramid.}
