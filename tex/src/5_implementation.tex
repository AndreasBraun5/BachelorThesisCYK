...
\end{center}
\caption{Example output for a $exercise$. Top: Context free grammar. Middle: A $pyramid$ filled after the CYK algorithm. Bottom: Exemplary random derivation tree.}
...
The first step here is the tokenization of the input in token as seen in Figure \ref{antlrGrammarTokens} on the right. After that with the help of the Grammar as seen in Figure \ref{antlrGrammarRules} an abstract syntax tree is generated out of which intern Java objects can be parsed for further processing.\\
\noindent
\begin{figure} [h]
	\centering
...
\noindent Hiermit erkläre ich, dass ich die Bachelorarbeit selbständig verfasst und keine anderen als die angegebenen Quellen und Hilfsmittel benutzt und die aus fremden Quellen direkt oder indirekt übernommenen Gedanken als solche kenntlich gemacht habe.
