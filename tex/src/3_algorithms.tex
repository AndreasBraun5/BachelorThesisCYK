...
\frame{
	\begin{algorithm}[H] %or another one check
		...
		\If{$Cell_{i,j} = \emptyset$}{ \label{empty}
			...
			$P = P \cup Distribute(VarComp,\ V) $; \circled{B} \label{splitThenFillB}  \\
		}
		
		\Return $(P,\ Cell_{i,j})$\;
		\footnotetext{
			\noindent Line \ref{cyk}: Needed to update the $pyramid$ to be able to check if the stopping criteria is met in Line \ref{cMet}.	
		}
	\end{algorithm}
}
\clearpage
...
\frame{
	\begin{algorithm}[H] %or another one check
		...
		\footnotetext{
			Line \ref{termDistr2}: There is the rule $v_{lhse}\rightarrow w_j$, then $v_{lhse}$ is the variable on the left side of the one rule that has the terminal $w_j$ as its rhse.
		}
	\end{algorithm}
}
\begin{testexample}[Algorithm SplitThenFill]
	...
	At first the rule $B\longrightarrow b$ is added as shown in Figure \ref{IllustrationAlgorithmSplitAndFillPart1}.\\
	\pagebreak
