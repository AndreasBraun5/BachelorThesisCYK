...
\frame{
	\begin{algorithm}[H] %or another one check
		...
		$P = KeepContributing(P)$\; \label{contributing}
		\Return $P$\;
		\footnotetext{ 
...
\end{center}
\end{table}
\noindent As seen in table \ref{DiceRollOnlySR} the algorithm shows a relatively low success rate for producibility. This can be explained with ... . Mention the basic connection between the success rates.
\paragraph{Conclusion} \mbox{} \\
\noindent Something about what can be improved in another attempt / the next attempt. Don't forget that a relatively small number of productions is already sufficient to completely fill the parsing table. Maybe remove not contributing productions in the the other algorithms too.
\pagebreak
