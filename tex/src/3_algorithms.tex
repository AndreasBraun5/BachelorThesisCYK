...
\frame{
	\begin{algorithm}[H] %or another one check
		...
		\KwOut{Set of rules $P \subseteq V \times V^{2}$ or $P \subseteq V \times \Sigma$}
...
\noindent \frame{
	\begin{algorithm}[H] %or another one check
		...
		\For{$k:=i-1 \to 0$}{ \label{cellCombs}
			$CellSet = CellSet \cup \{YZ\ |\ X\longrightarrow YZ,\ (Y,...) \in Cell_{k,j},\ (Z,...) \in Cell_{i-k-1,k+j+1} \}$\; \label{cellSet}
		}
		\Return $CellSet$;
...
\begin{testexample}[Algorithm CalculateSubsetForCell]
			In the following situation a rule is added to $P$ while using while using Algorithm \ref{CalculateSubsetForCell} on $Cell_{3,0}$.\\
	\begin{minipage}{6in}
		\centering
				\raisebox{-0.5\height}{
					\begin{tabular}{l}
...
\end{testexample}~\\
In general if for one $Cell_{i,j}$ a rule like $lhse \rightarrow cs$ with $cs \in CellSet$ (Line~\ref{cellSet}) is added, then automatically $Cell_{i,j}$ is not empty any more.
...
\frame{
	\begin{algorithm}[H] %or another one check
		...
		\KwOut{Set of rules $P$}
		$P = \emptyset$;~~\tcp{$P \subseteq V \times (V^{2} \cup \Sigma)$}
		$P = P \cup Distribute(\Sigma,\ V)$;  \circled{A}\\ \label{diceRollOnlyA}
		$P = P \cup Distribute(V^2,\ V)$;  \circled{B}\\ \label{diceRollOnlyB}
		\Return $P$;
	\end{algorithm}
}
...
\begin{testexample}[Algorithm DiceRollOnlyCYK]
	...
	\end{minipage}
	\captionof{figure}{Shortened overview of the example of Algorithm \ref{DiceRollOnlyCYK}.}
...
\begin{testexample}[Disregarding already added rules]
	Figure \ref{var1ExampleTop} shows the starting situation for this example:
	   ...
	   \centering
		...
		\end{minipage}
	\captionof{figure}{Disregarding the already added rules: Unfortunate adding.}
	...
	\begin{algorithm}[H] %or another one check
		...
		\For{$i:=1\ \textbf{to}\ i_{max}$}{
			...
			\For{$j:=0\ \textbf{to}\ j_{max}-i$}{
				$RowSet = RowSet \cup \{(xy,i)\ |\ xy \in CalculateSubsetForCell(Pyramid,\ i,\ j) $\circled{D}$\}$\label{rowSet}\;  
			}
			\While{$threshold_i\ not\ reached $}{ \label{var2threshold}
				$choose~ one~ (xy,i)~ from~ RowSet~ uniformly\ randomly\ with$ $probability~depending~on~i $;\label{chooseVc} \circled{E}  \\
...
\noindent \textbf{Choose one xy from (xy,i) $\in$ RowSet uniformly randomly with probability depending on row i \circled{E} :}\\
...
The approach here only makes sense together with \circled{D} where all possible compound variables are calculated that would help to fill one specific cell. $RowSet \subseteq \{(xy,i)\ |\ x,y \in V \wedge i \in \mathbb{N} \}$ where the $xy$ are calculated with \circled{D} and $i$ is the row number of the specific cell. \pagebreak \clearpage
...
\begin{testexample}[Derivation structure in the pyramid and as a derivation tree]
	...
	\end{minipage}
	\captionof{figure}{Derivation structure shown as a pyramid in the top and as a derivation tree below.}
	\label{treeStruct}
\end{testexample}~\\

\noindent As the name of the algorithms implies only after completely generating the structure of the derivation tree (splitting of the word in sub words) the rules are added to the grammar that help filling the cells occurring in this derivation tree. The phrase CYK is included in the name because in every recursion the CYK algorithm is executed (Line \ref{cyk2} of Algorithm 8) once.\\

\noindent
\frame{
	\begin{algorithm}[H] %or another one check
		\caption{SplitThenFillCYK}
		\label{SplitThenFill}
		\SetAlgoLined
		\KwIn{Word $w \in \Sigma^{*}$}
		\KwOut{Set of rules $P$}
		...
		$Sol = SplitThenFillCYKRec(P,\ w,\ i_{max},\ 0)$\; \label{splitThenFillTreeStart}
		\Return $P_{Sol}$\;
		\footnotetext{
...
\noindent Now every time before adding a new rule (Algorithm \ref{SplitThenFillRecursion} SplitThenFillCYKRec Line \ref{splitThenFillB}) the already available information regarding the other rules is used to determine if a new rule is needed to fill this node of the derivation tree (Line \ref{empty} of Algorithm \ref{SplitThenFillRecursion} SplitThenFillCYKRec).\\

\frame{
	\begin{algorithm}[H] %or another one check
		\caption{SplitThenFillCYKRec}
		...
		$choose\ one\ m\ uniformly\ randomly\ in\ [j+1,\ j+i]$\;
		$(P,\ Cell_l) = SplitThenFillCYKRec(P,\ w,\ (m-j-1),\ j)$\label{left}\;
		$(P,\ Cell_r) = SplitThenFillCYKRec(P,\ w,\ (j+i-m),\ m)$\label{right}\;
		$Pyramid = CYK(G,\ w)$\label{cyk2}\;
		\If{$stopping\ criteria~met$~\circled{C}}{ \label{cMet}
			\Return $(P,\ Cell_{i,j})$\;
		}
		\If{$Cell_{i,j} = \emptyset$}{ \label{empty}
			...
			$foreach~vc\in VarComp~choose~v\in V~uniformly~randomly~and~add~the$ $~rule~v\longrightarrow vc~to~P$\; \label{splitThenFillB}
		}
		
		\Return $(P,\ Cell_{i,j})$\;
		\footnotetext{
			\noindent Line \ref{cyk2}: Needed to update the $pyramid$ to be able to check if the stopping criteria is met in Line \ref{cMet}.	
		}
	\end{algorithm}
}~\\

\begin{testexample}[Algorithm SplitThenFillCYK]
	...
	\noindent The situation after adding the terminals (Line \ref{splitThenStepii} in Algorithm \ref{SplitThenFill} SplitThenFillCYK) to the grammar is shown in Figure \ref{IllustrationAlgorithmSplitThenFillPart1}:\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 1 after adding the rules $A \rightarrow a$, $B \rightarrow b$ and $C \rightarrow c$.}
	\label{IllustrationAlgorithmSplitThenFillPart1}
	~
	
	\noindent After adding the rules for the terminals to the grammar the recursion step at $Cell_{1,1}$ is taken on. Now $Cell_l=\{B_2\}$ and $Cell_r=\{A_3\}$ and therefore $VarComp=\{BA\}$. Adding the rule $S \rightarrow BA$ leads to the following $Pyramid$ shown in Figure \ref{IllustrationAlgorithmSplitThenFillPart2}: \\
	...
	\begin{minipage}{6in}
		...
		\end{minipage}
		\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 2 after adding the rule $S\rightarrow BA$.}
		...
		\end{minipage}
		\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 3 after adding the rule $A\rightarrow BS$.}
		\label{IllustrationAlgorithmSplitThenFillPart3}
		~\\
		The next two steps are described analogously with Figure \ref{IllustrationAlgorithmSplitThenFillPart4} and with Figure \ref{IllustrationAlgorithmSplitThenFillPart5}. \\
		...
		\end{minipage}
		\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 4. The recursion step in $Cell_{1,3}$ is  resolved by adding the rule $B\rightarrow CB$.}
		...
		\end{minipage}
		\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 5. The recursion step in $Cell_{2,3}$ is  resolved by adding the rule $C\rightarrow BC$.}
		\label{IllustrationAlgorithmSplitThenFillPart5}
		~\\
		\noindent Finally the last recursion step that decides on the content of the root cell is shown in Figure \ref{IllustrationAlgorithmSplitThenFillPart6}. In this case the start variable is luckily contained in the root of the pyramid.\\
		...
		\end{minipage}
		\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 6. The recursion step in $Cell_{5,0}$ is  resolved by adding the rule $S\rightarrow AC$.}
...
\frame{
	\begin{algorithm}[H] %or another one check
		...
		$Sol = SplitThenFillRec(P,\ w,\ i_{max},\ 0)$\;
		\Return $P_{Sol}$\;
		\footnotetext{
	...
	\begin{algorithm}[H] %or another one check
		...
		\If{$i=0$}{
			\If{terminal $w_j$ not distributed yet}{
				$choose~v\in V~uniformly~randomly$\;
				\Return $(P \cup \{(v,~ w_j)\},\ v)$\; \label{termDistr1}	
			}
			\Return $(P,\ v_{lhse})$;~~\tcp{see note below} \label{termDistr2}
		}
		$choose\ one\ m\ uniform\ randomly\ in\ [j+1,\ j+i]$\;
		$(P,\ v_l) = SplitThenFillRec(P,\ w,\ (m-j-1),\ j)$\label{left}\;
		$(P,\ v_r) = SplitThenFillRec(P,\ w,\ (j+i-m),\ m)$\label{right}\;	
		\If{$i=i_{max}$}{
		...
		\footnotetext{
			Line \ref{termDistr2}: $v_{lhse} \longrightarrow w_j \in P$ at this point, as terminal $w_j$ was already distributed.
		}
	\end{algorithm}
}
\begin{testexample}[Algorithm SplitThenFill]
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 1. To resolve the recursion step that fills $Cell_{0,0}$ the rule $B\rightarrow b$ is added.}
	\label{IllustrationAlgorithmSplitAndFillPart1}~\\
	
	\noindent Next are the rules $A\rightarrow a$ and $S\rightarrow BA$ as seen in Figure \ref{IllustrationAlgorithmSplitAndFillPart2}. To resolve the recursion step that fills $Cell_{0,1}$ no rule is added because a rule $lhse\rightarrow b$ already exists. To fill $Cell_{0,2}$ the rule $A\rightarrow a$ and regarding $Cell_{1,1}$ the rule $S \rightarrow BA$ is added.\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 2. Adding of the rules \\$A\rightarrow a$ and $S\rightarrow BA$.}
	\label{IllustrationAlgorithmSplitAndFillPart2}
	~\\
	\pagebreak
	
	\noindent To fill the $Cell_{2,0}$ the rule $C \rightarrow BS$ is added, see Figure \ref{IllustrationAlgorithmSplitAndFillPart3}: \\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 4. Adding of the rules \\$C\rightarrow c$, $B \rightarrow CB$ and  $A\rightarrow BC$.}
	\label{IllustrationAlgorithmSplitAndFillPart4}
	~\\
	\pagebreak
	
	\noindent To fill the cell in the root a rule must be added that has the start variable as its $lhse$ which guarantees $w \in L(G)$. Here the rule $S\rightarrow CA$ is added.\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 5. Adding of the rule \\$S\rightarrow CA$.}
	\label{IllustrationAlgorithmSplitAndFillPart5}~\\
	
	\noindent Finally a comparison of Figure \ref{IllustrationAlgorithmSplitAndFillPart5} and Figure \ref{IllustrationAlgorithmSplitAndFillPart6} shows the difference between the parsing table after the last step of the algorithm and the completely updated parsing table. Additional variables are found in the cells $Cell_{1,4}$, $Cell_{2,3}$, $Cell_{4,0}$ and $Cell_{5,0}$.\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 6. Comparison of the last step of the algorithm and the complete parsing table.}
...
\begin{testexample}[Simplification of cells in a pyramid]
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|c|c|c|c|c|c|}
		...
		DiceRollOnly                    & \textbf{0.9\%} & 18.2\%                                                           &10.2\%                                                              & 42.0\% &55.3\%                                                        &86.7\%                                                         &98.7\%                                                           \\ \hline
		BottomUpVar1                    &\textbf{4.3\%}      &26.9\%                                                           &47.1\%                                                              &41.4\%      &68.8\%                                                        &72.7\%                                                         &91.1\%                                                           \\ \hline
		BottomUpVar2                    &\textbf{6.0\%}      &38.1\%                                                           &30.3\%                                                              &47.2\%      &88.6\%                                                        &57.9\%                                                         &90.1\%                                                           \\ \hline
