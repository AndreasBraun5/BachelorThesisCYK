...
\begin{testexample}[Derivation structure in the pyramid and as a derivation tree]
	...
	\end{minipage}
	\captionof{figure}{Derivation structure shown in the pyramid in the top and below as a derivation tree.}
	\label{treeStruct}
\end{testexample}~\\

\noindent As the name of the algorithms implies only after completely generating the structure of the derivation tree (splitting of the word in sub words) the rules are added to the grammar that help filling the cells occurring in the derivation tree. The phrase CYK is included in the name because in every recursion the CYK algorithm is executed (Line \ref{cyk2}) one time.\\
Now every time before adding a new rule (Algorithm \ref{SplitThenFillRecursion} SplitThenFillCYKRec Line \ref{splitThenFillB}) the already available information regarding the other rules is used to determine if a new rule is needed to fill this node of the derivation tree (Line \ref{empty} of Algorithm \ref{SplitThenFillRecursion} SplitThenFillCYKRec).\\
...
\begin{testexample}[Algorithm SplitThenFillCYK]
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 1 after adding $A \rightarrow a$, $B \rightarrow b$ and $C \rightarrow c$.}
	...
	\begin{minipage}{6in}
		...
		\end{minipage}
		\captionof{figure}{Illustration of Algorithm \ref{SplitThenFill} SplitThenFillCYK part 2 after adding $S\rightarrow BA$.}
...
\begin{testexample}[Algorithm SplitThenFill]
	...
	\noindent Next are the rules $A\longrightarrow a$ and $S\longrightarrow BA$ as seen in Figure \ref{IllustrationAlgorithmSplitAndFillPart2}. To resolve the recursion step that fills $Cell_{0,1}$ no rule is added because a rule $lhse\rightarrow b$ already exists. To fill $Cell_{0,2}$ the rule $A\rightarrow a$ and regarding $Cell_{1,1}$ the rule $S \rightarrow BA$ is added.\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 2. Adding of the rules $A\longrightarrow a$ and $S\longrightarrow BA$.}
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 3. Adding of the rule $C \longrightarrow BS$ to fill the $Cell_{2,0}$.}
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 4. Adding of the rules $C\longrightarrow c$, $B \longrightarrow CB$ and  $A\longrightarrow BC$.}
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitThenFill part 5. Adding of the rule $S\longrightarrow CA$.}
