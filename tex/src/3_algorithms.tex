...
\noindent Now that the choice of stopping criteria has been decided for RootNotEmpty, that guarantees the better $SR$s it is time to choose the one algorithm for usage in the application.\\
\noindent Before deciding, a closer look onto the actual input values is given in the following Table \ref{inputValuesOfBestSuccessRate}. It might be interesting to see which configurations of input values of the algorithms are responsible for the good SR.
...
\end{table}
\noindent Nearly every time the count of variables and the count of terminals is three and the size of the word ranges from 8 to 9.\\
\clearpage
\noindent The composite success rates of the five algorithms are displayed in more detail in Table \ref{comparisionsAlgorithms}. While looking at the configuration of the algorithms that generate the best $SR$s, it is seen that the best algorithm for the application to use is Algorithm \ref{SplitAndFill} SplitAndFill, because it has the best $SR$.
