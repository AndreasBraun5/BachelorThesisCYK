...
\begin{testexample}[Algorithm SplitAndFill]
	According to this algorithm, only rules corresponding to the tree structure are added to the grammar. For illustration purposes, the pyramid is  shown to reflect the immediate changes of the added rules to the pyramid in the Figures \ref{IllustrationAlgorithmSplitAndFillPart1} to \ref{IllustrationAlgorithmSplitAndFillPart9}. Again the predefined derivation tree structure of Figure \ref{treeStruct} is used.\\
	At first always a rule like $S\longrightarrow vc$, where $S$ is the start variable and $vc\in V^2$, is added so that the root of the pyramid always contains the start variable and $w\in L(G)$. In this case it is the rule $S\longrightarrow CA$ as seen in Figure \ref{IllustrationAlgorithmSplitAndFillPart1}:
	\vspace{5cm}
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 1. To resolve the recursion step that fills $Cell_{i_{max},0}=Cell_{5,0}$ the rule $S\rightarrow CA$ is added.}
	\label{IllustrationAlgorithmSplitAndFillPart1}~\\
	
	\noindent Next is the rule $C\longrightarrow BS$ as seen in Figure \ref{IllustrationAlgorithmSplitAndFillPart2}. With this the $Cell_{2,0}$ contains the variable $C$:\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 2. Adding of the rule $C\longrightarrow BS$ to fill $Cell_{2,0}$.}
	\label{IllustrationAlgorithmSplitAndFillPart2}
	~\\
	\pagebreak
	
	\noindent To ensure that $Cell_{0,0}$ contains the variable $B$ the rule $B \rightarrow b$ is added as shown in Figure \ref{IllustrationAlgorithmSplitAndFillPart3}: \\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 3. Adding of the rule $B \longrightarrow b$ so that $Cell_{0,0}$ contains the variable $B$.}
	\label{IllustrationAlgorithmSplitAndFillPart3}~\\

	
	\noindent Next is to fill $Cell_{1,1}$ through adding the rule $S\longrightarrow BA$ as seen in Figure \ref{IllustrationAlgorithmSplitAndFillPart4}:  \\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 4. Adding of the rule $S\longrightarrow BA$.}
	\label{IllustrationAlgorithmSplitAndFillPart4}
	~\\
	\pagebreak
	
	\noindent $Cell_{0,1}$ contains the variable $B$ but no additional rule needs to be added because the terminal $b$ is already a $rhse$ of the rule $B \longrightarrow b$. To ensure that $Cell_{0,2}$ contains the variable $a$ the rule $A\longrightarrow a$ is added.\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 5. Adding of the rule $A\longrightarrow a$.}
	\label{IllustrationAlgorithmSplitAndFillPart5}~\\

	\noindent The next step is make sure that $Cell_{2,3}$ contains the variable $A$. For that the rule $A\longrightarrow BC$ is added.\\
		
	\begin{minipage}{6in}
		\centering
		\begin{tabular}{l}
			Grammar:\\
			$A \rightarrow \mathbf{BC}~|~a$\\
			$B \rightarrow b$\\ 
			$C \rightarrow BS$\\
			$S \rightarrow BA~|~CA$\\ 
		\end{tabular} 
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 6. Adding of the rule $A\longrightarrow BC$.}
	\label{IllustrationAlgorithmSplitAndFillPart6}~\\
	
	\pagebreak
	
	\noindent Now it is important again to make sure that the $Cell_{0,5}$ contains the variable $C$ and therefore the rule $C\longrightarrow c$ is added.\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 7. Adding of the rule $C\longrightarrow c$.}
	\label{IllustrationAlgorithmSplitAndFillPart7}~\\
	
	\noindent To ensure that $Cell_{1,3}$ contains the variable $B$ a rule is needed again. The terminal $c$ and $b$ already are a $rhse$ of the rules $C\longrightarrow c$ and $B\longrightarrow b$. It follows that the cell $Cell_{0,3}$ contains the variable $C$ and the cell $Cell_{0,4}$ the variable $B$. Because of this the rule $B \longrightarrow CB$ is added as seen in Figure \ref{IllustrationAlgorithmSplitAndFillPart8}\\
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 8. Adding of the rule $B\longrightarrow CB$.}
	\label{IllustrationAlgorithmSplitAndFillPart8}~\\
	
	\pagebreak
	
	\noindent Finally a comparison of Figure \ref{IllustrationAlgorithmSplitAndFillPart8} and Figure \ref{IllustrationAlgorithmSplitAndFillPart9} shows the difference between the intended parsing table up till now and the actual completely with the CYK algorithm updated parsing table. Additional variables are found in the cells $Cell_{1,4}$, $Cell_{2,3}$, $Cell_{4,0}$ and $Cell_{5,0}$.\\
	...
	\begin{minipage}{6in}
		...
		\resizebox{0.65\linewidth}{!}{
			...
			\coordinate (center23) at (4.5,-1.0);
			\node [below=0.18cm] at (center23) {\myfontnumbering{$(23)$}};
			\node [] at (center23) {\myfontvars{S$_4$A$_{5}$}};
			%cells30
			\coordinate (center30) at (2.0,-1.5);
			\node [below=0.18cm] at (center30) {\myfontnumbering{$(30)$}};
			%cells31
	...
	\end{minipage}
	\captionof{figure}{Illustration of Algorithm \ref{SplitAndFill} SplitAndFill part 9. Comparison of the last step of the algorithm and the complete parsing table.}
	\label{IllustrationAlgorithmSplitAndFillPart9}
