...
\noindent \textbf{CalculateSubsetForCell(Pyramid, i, j) \circled{E}:}\\
...
\begin{figure}[h]
	...
	\end{minipage}
\end{figure}\\
Calculation of CellSet for $Cell_{3,0}$ results in $\{SA,~SC,~BS\}$, whereas $SA$ and $SC$ stem from $Cell_{1,0}$ together with $Cell_{1,2}$ and $BA$ is from $Cell_{0,0}$ together with $Cell_{2,1}$. Now if either one of the rules $lhse \rightarrow SA$, $lhse \rightarrow SC$ or  $lhse \rightarrow BS$ is added to the grammar, then $lhse\in Cell_{3,0}$.
	...
	\begin{algorithm}[H] %or another one check
		...
		\For{$i:=1\ \textbf{to}\ i_{max}$}{
			...
			\While{$|J|>0$}{
				...
				$J = J \setminus \{j\} $\;
				$CellSet = CalculateSubsetForCell(Pyramid,\ i,\ j)$;  \circled{E}
	...
	\begin{algorithm}[H] %or another one check
		...
		\For{$i:=1\ \textbf{to}\ i_{max}$}{
			...
			\For{$j:=0\ \textbf{to}\ j_{max}-i$}{
				$RowSet \cup \{(XY,i)\ |\ XY \in CalculateSubsetForCell(Pyramid,\ i,\ j) $\circled{E}$\}$\label{rowSet}\;  
			}
