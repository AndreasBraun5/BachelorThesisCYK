...
\end{itemize}
\vspace{0.6cm}
\noindent The resulting best SRs of each algorithm are used to decide which stopping criteria \circled{C} gives more exam suitable $exercise$s and finally the algorithms are compared to each other. The best SR is calculated via BruteForce over all count of variables ascending again over all count terminals ascending and over the size of the word ascending. The best SR is only updated if the net determined SR is better than the tempBestOne. This means the first maximum (ascending) will be picked and all other maxima with the same height will be ignored. \\
...
\end{table}
\noindent It can be seen that the stopping criteria RootNotEmpty is better or at least as good as MoreThanHalf in all cases. All other discussion is done with the Criteria RootNotEmpty.
...
\end{table}
Mostly the count of variables and the count of terminals is three. No further discussion is done. The size of the word is 8 or 9. Don't know why.
\noindent By comparing the five algorithms in Table \ref{comparisionsAlgorithms} in more detail it is clearly seen that "very very good" SR are achieved. VarsInPyramid and VarsPerCell are in most cases fulfilled. ForceRight is kind of not always fulfilled. Why do I have to discuss this....
\begin{table}[h]
	\centering
