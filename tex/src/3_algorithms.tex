...
Two kinds of \circled{C} have been used. One is that it is true iff more than half of the pyramid cells are not empty and the other one is that there is at least one variable in the tip of the pyramid. Both stopping criteria are compared shortly in Chapter \ref{AnalysisOfAlgorithms}.
...
\begin{figure}[h]
	...
	\end{minipage}
	\caption{Example of Algorithm \ref{CalculateSubsetForCell} while applying it on $Cell_{3,0}$.}
\end{figure}\\
...
\begin{figure}[h]
	...
	\end{minipage}
	\caption{In between step of Algorithm \ref{BottomUpDiceRollVar1}, disregarding the already added rules.}
\end{figure}\\
...
During the comparison of the algorithms the n-dimensional problem spaced is reduced to a single scalar value, the Success Rate.\\
